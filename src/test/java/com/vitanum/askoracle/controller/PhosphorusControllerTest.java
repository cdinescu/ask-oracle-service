...
@WebFluxTest(PhosphorusController.class)
public class PhosphorusControllerTest {
    public static final String BASE_PATH = "/top-richest-foods/phosphorus";
    ...
    public void getRichestFoods() {
        // Arrange
        Phosphorus entity = new Phosphorus();
        EntityUtils.decorateEntity(entity);

        List<Phosphorus> entries = Arrays.asList(entity);
        Mockito.when(repository.findByOrderByQuantityDesc(Mockito.any(Pageable.class))).thenReturn(Flux.fromIterable(entries));

        // Act & Assert
        WebClientTestUtils.sendRequestAndVerify(webTestClient, BASE_PATH, Phosphorus.class, entries);
    }
