...
@WebFluxTest(ManganeseController.class)
@DirtiesContext
public class ManganeseControllerTest {
    public static final String BASE_PATH = "/top-richest-foods/manganese";
    ...
    public void getRichestFoods() {
        // Arrange
        Manganese entity = new Manganese();
        EntityUtils.decorateEntity(entity);

        List<Manganese> entries = Arrays.asList(entity);
        Mockito.when(repository.findByOrderByQuantityDesc(Mockito.any(Pageable.class))).thenReturn(Flux.fromIterable(entries));

        // Act & Assert
        WebClientTestUtils.sendRequestAndVerify(webTestClient, BASE_PATH, Manganese.class, entries);
    }
