...
@WebFluxTest(IronController.class)
@DirtiesContext
public class IronControllerTest {
    public static final String BASE_PATH = "/top-richest-foods/iron";
    ...
    public void getRichestFoods() {
        ...
        List<Iron> entries = Arrays.asList(entity);
        Mockito.when(repository.findByOrderByQuantityDesc(Mockito.any(Pageable.class))).thenReturn(Flux.fromIterable(entries));

        // Act & Assert
        WebClientTestUtils.sendRequestAndVerify(webTestClient, BASE_PATH, Iron.class, entries);
    }
