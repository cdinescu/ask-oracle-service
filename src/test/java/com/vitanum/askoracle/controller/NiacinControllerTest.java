...
@WebFluxTest(NiacinController.class)
public class NiacinControllerTest {
    public static final String BASE_PATH = "/top-richest-foods/niacin";
    ...
    public void getRichestFoods() {
        // Arrange
        Niacin entity = new Niacin();
        EntityUtils.decorateEntity(entity);

        List<Niacin> entries = Arrays.asList(entity);
        Mockito.when(repository.findByOrderByQuantityDesc(Mockito.any(Pageable.class))).thenReturn(Flux.fromIterable(entries));

        // Act & Assert
        WebClientTestUtils.sendRequestAndVerify(webTestClient, BASE_PATH, Niacin.class, entries);
    }
